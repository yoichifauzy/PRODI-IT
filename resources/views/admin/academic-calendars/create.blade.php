...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800">Tambah Kegiatan Akademik</h1>
    <p class="text-sm text-slate-500">Jadwal yang ditambahkan akan otomatis tersinkronisasi dan tampil di Google Calendar publik.</p>
</div>

<form method="POST" action="{{ route('admin.academic-calendars.store') }}" class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
    @csrf
    
    <div class="space-y-5">
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Judul Kegiatan <span class="text-rose-500">*</span></label>
            <input type="text" name="title" required value="{{ old('title') }}" class="w-full rounded-lg border border-slate-200 px-4 py-2 text-sm focus:border-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-800" placeholder="Misal: Ujian Akhir Semester Genap">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="date" name="start_date" required value="{{ old('start_date') }}" class="w-full rounded-lg border border-slate-200 px-4 py-2 text-sm focus:border-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-800">
            ...
            <div>
                ...
                <input type="date" name="end_date" value="{{ old('end_date') }}" class="w-full rounded-lg border border-slate-200 px-4 py-2 text-sm focus:border-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-800">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border border-slate-200 px-4 py-2 text-sm focus:border-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-800" placeholder="Keterangan tambahan mengenai kegiatan ini...">{{ old('description') }}</textarea>
    ...
    <div class="mt-8 flex items-center gap-3 border-t border-slate-100 pt-5">
        <button type="submit" class="rounded-lg bg-slate-900 px-5 py-2 text-sm font-semibold text-white hover:bg-slate-800 transition">Simpan Jadwal</button>
