...
@section('content')
    <div class="mb-4">
        <h1 class="text-2xl font-bold text-slate-900">Edit Item Galeri</h1>
        <p class="text-sm text-slate-600">Perbarui data gambar dokumentasi kegiatan.</p>
    </div>

    <form method="POST" action="{{ route('admin.gallery-items.update', $galleryItem) }}" enctype="multipart/form-data" class="rounded-xl border border-slate-200 bg-white p-4">
        @csrf
        @method('PUT')
